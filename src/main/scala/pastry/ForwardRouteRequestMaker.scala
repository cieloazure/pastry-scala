...
object ForwardRouteRequestMaker {
  def props(to: Entry,
            key: String,
            trace: Array[Entry],
            state: (LeafSet[Entry], RoutingTable[Entry], NeighbourhoodSet[Entry]),
            host: Entry,
            respondTo: ActorRef): Props = Props(new ForwardRouteRequestMaker(to, key, trace, state, host, respondTo))
}

class ForwardRouteRequestMaker(to: Entry,
                              key: String,
                              trace: Array[Entry],
                              state: (LeafSet[Entry], RoutingTable[Entry], NeighbourhoodSet[Entry]),
                              host: Entry,
  ...
  to._actor ! ForwardRouteRequest(key, trace, respondTo)

  override def receive: Receive = {
    case ForwardRouteRequestAck =>
      context.stop(self)
    ...
    case ReceiveTimeout =>
      ...
      host._actor ! ForwardRouteRequest(key, oldTrace, respondTo)
  }
